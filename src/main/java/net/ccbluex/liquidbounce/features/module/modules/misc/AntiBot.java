...
@ModuleInfo(name = "AntiBot", spacedName = "Anti Bot", description = "", category = ModuleCategory.MISC)
    ...
    public void onUpdate(final UpdateEvent event) {
        ...
        if (removeFromWorld.get() && mc.thePlayer.ticksExisted > 0 && mc.thePlayer.ticksExisted % removeIntervalValue.get() == 0) {
            ...
            for (EntityPlayer e : ent) {
                ...
                if (debugValue.get())
                    ClientUtils.displayChatMessage("§7[§a§lAnti Bot§7] §fRemoved §r" + e.getName() + " §fdue to it being a bot.");
            }
        }
    }
            ...
            if (packet instanceof S38PacketPlayerListItem) {
                ...
                if (data.getProfile() != null && data.getProfile().getName() != null) {
                    if (!wasAdded)
                        ...
                        event.cancelEvent();
                        if (debugValue.get())
                            ClientUtils.displayChatMessage("§7[§a§lAnti Bot/§6Matrix§7] §fPrevented §r" + data.getProfile().getName() + " §ffrom spawning.");
                    }
                }
            }
        }
