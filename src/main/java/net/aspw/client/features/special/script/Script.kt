package net.aspw.client.features.special.script
...
import net.aspw.client.features.special.script.api.ScriptCommand
import net.aspw.client.features.special.script.api.ScriptModule
import net.aspw.client.features.special.script.api.ScriptTab
import net.aspw.client.features.special.script.api.global.Chat
import net.aspw.client.features.special.script.api.global.Item
import net.aspw.client.features.special.script.api.global.Setting
...
class Script(val scriptFile: File) {
    ...
    private fun supportLegacyScripts() {
        if (getMagicComment("api_version") != "2") {
            ...
            val legacyScript =
                Client::class.java.getResource("/assets/minecraft/client/scriptapi/legacy.js").readText()
    ...
    fun onDisable() {
        ...
        registeredModules.forEach { Client.moduleManager.unregisterModule(it) }
        registeredCommands.forEach { Client.commandManager.unregisterCommand(it) }
    ...
    fun import(scriptFile: String) {
        scriptEngine.eval(File(Client.scriptManager.scriptsFolder, scriptFile).readText())
    }
