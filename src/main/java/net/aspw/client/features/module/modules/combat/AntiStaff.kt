    ...
    fun onPacket(event: PacketEvent) {
        ...
        if (packet is S1DPacketEntityEffect) {
            ...
            if (entity != null && (obStaffs.contains(entity.name) || obStaffs.contains(entity.displayName.unformattedText))) {
                if (!detected) {
                    Client.hud.addNotification(Notification("${entity.name} / effect.", Notification.Type.ERROR))
        ...
        if (packet is S14PacketEntity) {
            ...
            if (entity != null && (obStaffs.contains(entity.name) || obStaffs.contains(entity.displayName.unformattedText))) {
                if (!detected) {
                    Client.hud.addNotification(Notification("${entity.name} / update.", Notification.Type.ERROR))
                    mc.thePlayer.sendChatMessage("/leave")
                    detected = true
                }
            }
        ...
        if (packet is S19PacketEntityStatus) {
            ...
            if (entity != null && (obStaffs.contains(entity.name) || obStaffs.contains(entity.displayName.unformattedText))) {
                if (!detected) {
                    Client.hud.addNotification(Notification("${entity.name} / status.", Notification.Type.ERROR))
                    mc.thePlayer.sendChatMessage("/leave")
                    detected = true
                }
            }
        ...
        if (packet is S19PacketEntityHeadLook) {
            ...
            if (entity != null && (obStaffs.contains(entity.name) || obStaffs.contains(entity.displayName.unformattedText))) {
                if (!detected) {
                    Client.hud.addNotification(Notification("${entity.name} / head.", Notification.Type.ERROR))
                    mc.thePlayer.sendChatMessage("/leave")
                    detected = true
                }
            }
        ...
        if (packet is S49PacketUpdateEntityNBT) {
            ...
            if (entity != null && (obStaffs.contains(entity.name) || obStaffs.contains(entity.displayName.unformattedText))) {
                if (!detected) {
                    Client.hud.addNotification(Notification("${entity.name} / nbt.", Notification.Type.ERROR))
                    mc.thePlayer.sendChatMessage("/leave")
                    detected = true
                }
            }
