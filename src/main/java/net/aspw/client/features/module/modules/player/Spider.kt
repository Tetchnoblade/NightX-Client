...
class Spider : Module() {
    private val modeValue =
        ListValue(
            "Mode",
            arrayOf("Simple", "Jump", "PurplePrison", "CheckerClimb", "Clip", "AAC3.3.12", "AACGlide", "Verus"),
            "Simple"
        )
    ...
    fun onUpdate(event: MotionEvent) {
        ...
        when (modeValue.get().lowercase(Locale.getDefault())) {
            ...
            "purpleprison" -> if (mc.thePlayer.isCollidedHorizontally && !mc.thePlayer.isOnLadder) {
                waited++
                if (waited == 1) mc.thePlayer.motionY = 0.43
                if (waited == 12) mc.thePlayer.motionY = 0.43
                if (waited == 23) mc.thePlayer.motionY = 0.43
