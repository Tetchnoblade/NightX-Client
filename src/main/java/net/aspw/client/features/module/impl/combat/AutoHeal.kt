    ...
    fun onMotion(event: MotionEvent) {
        if (autoPotValue.get()) {
            if (event.eventState == EventState.PRE) {
                ...
                if (throwing && !mc.thePlayer.isEating && !mc.thePlayer.isInWater && MovementUtils.isRidingBlock() && mc.inGameHasFocus && Display.isActive() && mc.currentScreen !is GuiContainer && (!killAura?.state!! || killAura.target == null) && !scaffold?.state!! && !killAuraRecode?.isTargeting!! && !legitScaffold?.state!!) {
        ...
        if (autoPotValue.get()) {
            if (event.eventState == EventState.POST) {
                ...
                if (throwing && mc.currentScreen !is GuiContainer
                    ...
                    && tickTimer.hasTimePassed(4) && (!killAura?.state!! || killAura.target == null) && !scaffold?.state!! && !killAuraRecode?.isTargeting!! && !legitScaffold?.state!!
                ) {
