...
object SettingsUtils {
    ...
    fun generateScript(values: Boolean, binds: Boolean, states: Boolean): String {
        ...
        Client.moduleManager.modules.filter { it.category !== ModuleCategory.BETA && it !is SilentView && it !is Gui && it !is OptiFinePlus && it !is Cape && it !is HackerDetect && it !is HudEditor && it !is XRay && it !is Wings && it !is BlockOverlay && it !is ChestESP && it !is Crosshair && it !is Jenisuke && it !is CustomModel && it !is EnchantColor && it !is MoreParticles && it !is ItemESP && it !is ItemPhysics && it !is JumpCircle && it !is PlayerEdit && it !is PointerESP && it !is ViewBobbing && it !is ChinaHat && it !is NameProtect && it !is Tracers && it !is Trails && it !is Freecam && it !is FreeLook && it !is Plugins && it !is ThunderNotifier && it !is Hud && it !is Animations }
            .forEach {
                if (values)
