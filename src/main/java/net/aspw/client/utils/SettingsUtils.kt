...
object SettingsUtils {
    ...
    fun generateScript(values: Boolean, binds: Boolean, states: Boolean): String {
        ...
        Client.moduleManager.modules.filter { it.category !== ModuleCategory.RENDER && it !is Cape && it !is LagBack && it !is Plugins && it !is Wings && it !is ThunderNotifier && it !is AntiVanish && it !is HackerDetector && it !is SilentView && it !is Hud && it !is NoInvClose && it !is HudEditor && it !is Gui && it !is Animations }
            .forEach {
                if (values)
