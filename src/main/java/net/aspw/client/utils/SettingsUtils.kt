...
object SettingsUtils {
    ...
    fun generateScript(values: Boolean, binds: Boolean, states: Boolean): String {
        ...
        Client.moduleManager.modules.filter { it.category !== ModuleCategory.RENDER && it !is Cape && it !is CustomModel && it !is FreeLook && it !is Plugins && it !is Wings && it !is ThunderNotifier && it !is AntiVanish && it !is HackerDetector && it !is SilentView && it !is Hud && it !is NoInvClose && it !is HudEditor && it !is Gui && it !is BlockAnimations }
            .forEach {
                if (values)
