...
class HideCommand : Command("hide", emptyArray()) {
        ...
        if (args.size > 1) {
            when {
                args[1].equals("list", true) -> {
                    chat("§c§lHidden")
                    NightX.moduleManager.modules.filter { !it.array }.forEach {
                ...
                args[1].equals("category", true) -> {
                    if (args.size < 3) {
                        ...
                        return
                    } else if (NightX.moduleManager.modules.find {
                            it.category.displayName.equals(
                                args[2],
                                true
                            )
                        } != null) {
                        NightX.moduleManager.modules.filter { it.category.displayName.equals(args[2], true) }
    ...
    override fun tabComplete(args: Array<String>): List<String> {
        ...
        when (args.size) {
            1 -> {
                val moduleList = NightX.moduleManager.modules
                    .map { it.name }
