...
class InventoryManager : Module() {
    ...
    fun performManager() {
        if ((noScaffoldValue.get() && NightX.moduleManager[Scaffold::class.java]!!.state) || !InventoryUtils.CLICK_TIMER.hasTimePassed(
                delay
            ) ||
    ...
    fun isUseful(itemStack: ItemStack, slot: Int): Boolean {
        return try {
            ...
            ClientUtils.getLogger().error("§c>>§f Failed to check item: ${itemStack.unlocalizedName}.", ex)
            true
        }
    }
