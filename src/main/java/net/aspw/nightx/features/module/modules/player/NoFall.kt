...
import net.aspw.nightx.utils.VecRotation
...
class NoFall : Module() {
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        if (!state || NightX.moduleManager[Freecam::class.java]!!.state
            || mc.thePlayer.isSpectator || mc.thePlayer.capabilities.allowFlying || mc.thePlayer.capabilities.disableDamage
        )
            return

        if (!NightX.moduleManager[Flight::class.java]!!.state && voidCheckValue.get() && !MovementUtils.isBlockUnder()) return
