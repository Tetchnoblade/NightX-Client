...
import net.aspw.nightx.ui.client.hud.element.Side.Horizontal
import net.aspw.nightx.ui.client.hud.element.Side.Vertical
...
class Arraylist(
    ...
    private val colorModeValue =
        ListValue("Color", arrayOf("Custom", "Random", "Sky", "CRainbow", "LiquidSlowly", "Fade", "Mixer"), "CRainbow")
    ...
    private val textHeightValue = FloatValue("TextHeight", 10.5F, 1F, 20F)
        ...
        for (module in sortedModules) {
            ...
            if (module.array && (module.state || module.slide != 0F)) {
                ...
                when (hAnimation.get()) {
                    ...
                    "Slide" -> {
                        if (module.state) {
                            if (module.slide < width) {
                                module.slide = net.aspw.nightx.utils.AnimationUtils.animate(
                                    width.toDouble(),
            ...
            if (module.array && module.slide > 0F) {
                ...
                when (vAnimation.get()) {
                    ...
                    "Slide", "Rise" -> module.arrayY = net.aspw.nightx.utils.AnimationUtils.animate(
                        yPos.toDouble(),
    ...
    override fun updateElement() {
        modules = if (abcOrder.get()) NightX.moduleManager.modules
            .filter { it.array && (if (hAnimation.get().equals("none", ignoreCase = true)) it.state else it.slide > 0) }
        else NightX.moduleManager.modules
            .filter { it.array && (if (hAnimation.get().equals("none", ignoreCase = true)) it.state else it.slide > 0) }
            .sortedBy { -fontValue.get().getStringWidth(getModName(it)) }
        sortedModules = if (abcOrder.get()) NightX.moduleManager.modules.toList()
        else NightX.moduleManager.modules.sortedBy { -fontValue.get().getStringWidth(getModName(it)) }.toList()
    }
