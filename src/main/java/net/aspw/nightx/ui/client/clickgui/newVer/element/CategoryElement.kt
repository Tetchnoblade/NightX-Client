package net.aspw.nightx.ui.client.clickgui.newVer.element

import net.aspw.nightx.NightX
import net.aspw.nightx.features.module.ModuleCategory
import net.aspw.nightx.ui.client.clickgui.newVer.ColorManager
import net.aspw.nightx.ui.client.clickgui.newVer.element.module.ModuleElement
...
class CategoryElement(val category: ModuleCategory) : MinecraftInstance() {
    ...
    init {
        NightX.moduleManager.modules.filter { it.category == category }
    ...
    fun drawPanel(mX: Int, mY: Int, x: Float, y: Float, width: Float, height: Float, wheel: Int, accentColor: Color) {
        ...
        for (moduleElement in moduleElements) {
            if (startY + animScrollHeight > y + height || startY + animScrollHeight + 40F + moduleElement.animHeight < y + 50F)
                ...
                startY += moduleElement.drawElement(
                    mouseX,
                    mouseY,
                    x,
                    startY + animScrollHeight,
                    width,
                    40F,
                    accentColor
                )
