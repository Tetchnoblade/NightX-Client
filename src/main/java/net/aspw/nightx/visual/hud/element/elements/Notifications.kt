...
import net.aspw.nightx.NightX.hud
import net.aspw.nightx.utils.render.AnimationUtils
    ...
    fun drawNotification(animationY: Float, parent: Notifications) {
        ...
        if (vAnimMode.equals("smooth", true)) {
            if (firstY == 19190.0F)
                ...
                firstY = net.aspw.nightx.utils.AnimationUtils.animate(animationY, firstY, 0.02F * delta)
        } else {
            firstY = animationY
        }
        ...
        when (fadeState) {
            FadeState.IN -> {
                if (x < width) {
                    if (hAnimMode.equals("smooth", true))
                        x = net.aspw.nightx.utils.AnimationUtils.animate(width, x, animSpeed * 0.025F * delta)
                    else
            ...
            FadeState.OUT -> if (x > 0) {
                if (hAnimMode.equals("smooth", true))
                    x = net.aspw.nightx.utils.AnimationUtils.animate(
                        -width / 2F,
                        x,
